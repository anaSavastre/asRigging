...
class rigSceneSetup(object):
    def loadLatestFile(self, path):
        ...
        mc.file( path+"/"+latestFile, i= True, type= "mayaAscii", usingNamespaces= True, f=True)
    def returnLatertVersion(self, path):
        '''
        This function gets all the files in the given directory and loads the latest maya scene file
        '''
        print "return latest fct"
        files= os.listdir(path)
        latestFile = sorted(files)[-1];
    ...
    def __init__(self, rigName, projectEnv):
        ...
        modelFolferPath = projectEnv+"/modelling/"+rigName+"/scenes"
        latestModelVesion  = self.returnLatertVersion(modelFolferPath)
        ...
        # IMPORT MODEL
        mc.file(latestModelVesion, i= True, type= "mayaAscii", f=True)
...
# # MAIN
class roadSign(rigSceneSetup):
    env = "roadSign"
    def __init__(self, rigName, projectEnv):
        super(roadSign, self).__init__(rigName, projectEnv)

        panelCtl=[]
        for guide in (fn.getChildren("C_signPanel00_GRP")):
            panelCtl.append(rigFn.constructCTL(guide, side="C", name="bind_signPanel", parent=self.rootJnt))

        mc.parentConstraint(self.rootJnt, "s1_signPost_GEO")
        for i, ctl in enumerate(panelCtl):
            mc.parentConstraint(ctl, "s1_signPanel0" +str(i)+"_GEO", mo=True)

        mc.delete("C_signPanel00_GRP")
