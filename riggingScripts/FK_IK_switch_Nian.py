...
def renameDescendents(joints):
    ...
    PROCEDURE:
        ...
        Second:    Descendent for loor ( cmds.listRelatives() function returns the children in a list in which the first element is the 
                    ...
                    Inside the loop the function creates a string that holds the name to assign to the current joint 
                        basically it replaces the "bind_" or "be_" prefix to "IK_" or "FK_"
    ...
    for index in range (len(descendentsList)-1,-1, -1):
        ...
        #the bind_* joints which will be renamed to IK_*, the be_ which will become be_IK*, and the effector will not be touches (continue)
        current_string=''
        begin= 5#the begin position from which we will start the string
        if ("be_" in elem):
            #end joint case 
            begin=3
            #initialising the current_string to hold the prefix: "be_FK_" or "be_IK_"
            current_string='be_'+joints[0]+joints[1]+'_'  #    joints[0] + joints [1] = "FK" / "IK"
...
def createLocator(jointList, globalNode):
    ...
    cmds.parent ("FK_IK_Switches", globalNode)
...
def createStr_for_DrivKey(switches, jointRoots):
    ...
    if (jointRoots[0]=="b" and jointRoots[1]=="e"):
        FKconstraint=jointRoots+"_orientConstraint1."+concat_str("be_FK_", jointRoots,3)+"W0"
        IKconstraint=jointRoots+"_orientConstraint1."+concat_str("be_IK_", jointRoots,3)+"W1"
    #bind joint case:    
    else:
...
#ask user to introduce name of Global node 

globalNode='Nian_Global_01'
...
createLocator(leg_root_joints, globalNode)
