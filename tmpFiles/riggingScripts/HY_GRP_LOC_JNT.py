...
def createHy_for_obj(jntObj):
    ...
    #placing locObj in the same point as the joint
    snapObj(jntObj, locObj)
    
    #creating group
    grp=cmds.group(em=True, n=concat_str('grp', jntObj, concatPrefix))
    
    #aligning group to joint
    constr=cmds.orientConstraint(jntObj, grp)
    cmds.delete(constr)
    ...
    #freezing transformation on loc obj
    cmds.makeIdentity(locObj, apply=True, t=True, r=True, s=True) 
    cmds.parent(jntObj, locObj)
